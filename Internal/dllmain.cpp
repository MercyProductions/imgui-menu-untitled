...
namespace imgui
{
	...
	ImFont* start(IDXGISwapChain3* pSwapChain, ID3D12CommandQueue* pCommandQueue, type::tImguiStyle SetStyleFunction)
	{
		...
		ImFont* main_font = io.Fonts->AddFontFromMemoryTTF(poppins, sizeof(poppins), 21.f, nullptr, io.Fonts->GetGlyphRangesCyrillic());
		font::poppins = io.Fonts->AddFontFromMemoryTTF(poppins, sizeof(poppins), 21.f, NULL, io.Fonts->GetGlyphRangesCyrillic());
		font::icon = io.Fonts->AddFontFromMemoryTTF(icon, sizeof(icon), 21.f, NULL, io.Fonts->GetGlyphRangesCyrillic());
		font::icon_combo = io.Fonts->AddFontFromMemoryTTF(icon, sizeof(icon), 15.f, NULL, io.Fonts->GetGlyphRangesCyrillic());
	...
	ImFont* add_font(const char* font_path, float font_size)
	{
		...
		ImGuiIO& io = ImGui::GetIO();
		ImFont* font = io.Fonts->AddFontFromMemoryTTF(poppins, sizeof(poppins), 21.f, nullptr, io.Fonts->GetGlyphRangesCyrillic());
